...
def cancel_gradients_last_layer(epoch, model, freeze_last_layer):
    ...
    for n, p in model.named_parameters():
        if "last_layer" in n:
            p.grad = None
def only_load_model(ckp_path, **kwargs):
    ...
    for key, value in kwargs.items():
        if key in checkpoint and value is not None:
            try:
                ...
                print("=> only loaded '{}' from checkpoint '{}' with msg {} for eval".format(key, ckp_path, msg))
            except TypeError:
                try:
    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:   #为啥不直接全部cat？ 因为global 和 local crop size不一样！！！
            ...
            if isinstance(_out, tuple):
                _out = _out[0]
            # accumulate outputs
            output = torch.cat((output, _out))    # (?, dim)
            start_idx = end_idx
        # Run the head forward on the concatenated features.
        return self.head(output) #(b * n_crops, dim)
...
def get_params_groups(model):
    ...
    return [{'params': regularized}, {'params': not_regularized, 'weight_decay': 0.}] #只有第一组不是偏置的参数需要 weight_decay schedual
